...
public class MousePicker {
  ...
  public Vector3f getCurrentRay(Window window) {
    return currentRay;
  ...
  private Vector3f calculateMouseRay(Window window) {
    ...
    Vector2f normalisedCoordinates = getNormalisedDeviceCoordinates(window, mouseX, mouseY);
...
//  }
  ...
  private Vector2f getNormalisedDeviceCoordinates(Window window, float mouseX, float mouseY) {
    float normalisedX = (2f * mouseX) / window.getWidth() - 1f;
    float normalisedY = (2f * mouseY) / window.getHeight() - 1f;
